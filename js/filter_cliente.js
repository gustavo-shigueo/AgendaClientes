...
const clientList = document.querySelector('.clients-body')
const newList    = []
    ...
    for(item of arr){
        const child = `
            <div class=\"row\" data-id=\"${item.ID}\">
                ...
                <div class=\"column\" data-cpf>${item.cpf}<a href=\"visualizar_cliente.php?id=${item.ID}\"><i class=\"fas fa-caret-right\"></i></a></div>
...
searchBar.addEventListener('input', event => {
    ...
    const array  = newList.filter(item =>
        ...
        item.cpf.replace(/[^\d]/g, '').includes(search) ||
        item.fone.includes(search) ||
        item.fone.replace(/[^\d]/g, '').includes(search)
    )
    filterResults(array)
})